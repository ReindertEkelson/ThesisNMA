...
IV=function(X)
{
  ...
  mtc.model1 <- mtc.model(mtc.network, type ="consistency", likelihood = "binom", link = "logit", om.scale = 5,
                          hy.prior=mtc.hy.prior("std.dev", "dunif", 0, 5),  n.chain = "2",linearModel = "random")
...
d_NMA_BayU = c()
for (i in 1:(NT-1)) {
  d_NMA_BayU[i] =mean(d[seq(i, length(d), NT-1)])}
D_NMA_BayU[r] = list(d_NMA_BayU)
...
CId_NMA_BayU = c()
for (i in 1:(NT-1)) {
  CId_NMA_BayU[i] =mean(CId[seq(i, length(CId), NT-1)])}
CID_NMA_BayU[r] = list(CId_NMA_BayU)
MeanCI_NMA_BayU[r] = mean(CId)

#Bias and RMSE
biasIV.RE=c()
for (i in 1:N.sim){  biasIV.RE=c(biasIV.RE, l1[[i]]$biasRE)}
BIAS_NMA_BayU[r] = mean(biasIV.RE) #Bias
RMSE_NMA_BayU[r]= sqrt(mean((biasIV.RE)^2)) #RMSE

#Coverage probability
coverageIV.RE=c()
for (i in 1:N.sim){  coverageIV.RE=c(coverageIV.RE, l1[[i]]$covRE)}
CP_NMA_BayU[r] = mean(coverageIV.RE) #CP

#Tau squared
TAU2.RE=c()
for (i in 1:N.sim){  TAU2.RE=c(TAU2.RE, l1[[i]]$Tau2RE)}
TAU2_NMA_BayU[r] = mean(TAU2.RE) #TAU2
